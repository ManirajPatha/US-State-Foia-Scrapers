...
"""
nevada.py — Send one Nevada public-records request email per Excel row.
Robust version: SSL option, retries, resume, pacing, xls/xlsx-safe reader.

Usage:
  # Preview only (no sending)
  python nevada.py --input "C:/path/nevada_closed_results.xlsx"

  # Actually send first 10 rows (STARTTLS 587 by default)
  python nevada.py --input "C:/path/nevada_closed_results.xlsx" --send --limit 10

  # Use SMTPS/SSL on 465 (often fixes 'Connection unexpectedly closed')
  python nevada.py --input "C:/path/bidSearchResults.xls" --send --ssl --pause 2

  # Test to yourself (no sending)
  python nevada.py --input "C:/path/nevada_closed_results.xlsx" --to-override "pathamaniraj97@gmail.com"

  # Resume without resending rows already SENT per a prior log
  python nevada.py --input "C:/path/bidSearchResults.xls" --send --resume-log "C:/path/bidSearchResults_send_log_YYYYMMDD_HHMM.xlsx"
...
DEFAULT_RECIPIENT = "DeptAdmin@admin.nv.gov"
...
def infer_subject(row: pd.Series) -> str:
    title = s(row.get("title")) or s(row.get("solicitation_title")) or "Opportunity"
    notice_id = s(row.get("notice_id")) or s(row.get("solicitation_number")) or ""
    suffix = f" (Notice {notice_id})" if notice_id else ""
    return f"Nevada Public Records Request – {title}{suffix} – {REQUESTER['organization']}"
...
To: Nevada Department of Administration – Public Records
...
Pursuant to the Nevada Public Records Act, I respectfully request access to the records related to the opportunity detailed below.
These details are taken from my attached opportunities list (this request refers to the specific row summarized here):
...
Preferred Delivery
------------------
I would like to receive the responsive records electronically at the email address above.
If any portion of this request is unclear or overbroad, please let me know so I can narrow it.

Fees & Timing
-------------
Please inform me in advance if fees exceed $50 and provide a cost breakdown.
If records are exempt in whole or in part, please cite the specific statutory exemptions and release all segregable portions.
...
# --------------------------------- Main ------------------------------------
def main():
    ap = argparse.ArgumentParser(description="Nevada PRR emailer (robust).")
    ...
    ap.add_argument("--to-override", help="Send to this address instead of DeptAdmin@admin.nv.gov.")
